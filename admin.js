// admin.js – полная админ-панель с управлением выводами и резервом
...
window.renderAdminTab = async function() {
    ...
    try {
        ...
        const withdrawals = await window.adminGetWithdrawals();
        ...
        let html = `
            <div class="admin-container">
                ...
                <div class="admin-nav">
                    ...
                    <button class="admin-nav-btn" data-tab="finance">💰 Финансы</button>
                ...
                <div id="admin-dashboard" class="admin-pane active">
                    <div class="stats-grid">
                        ...
                        <div class="stat-card"><div class="stat-icon">🏦</div><div class="stat-value">${(reserve / 100).toFixed(2)} ⭐</div><div class="stat-label">Резерв</div></div>
                    </div>
                ...
                <div id="admin-mm" class="admin-pane" style="display:none;">
                    ...
                    <div style="margin-bottom:16px;">
                        ...
                        <p><strong>Уровень майнинга:</strong> <span id="mmLevelDisplay">${await window.getMiningLevelForUser(999999999) || 1}</span></p>
                    </div>
                    <div>
                        <button id="runMMBtn" style="background:linear-gradient(135deg,#fbbf24,#f59e0b); color:#1e1e2f;">▶️ Запустить маркет-мейкера</button>
                        <p style="font-size:12px; color:#9ca3af; margin-top:8px;">Автоматический запуск каждые 5 минут</p>
                    </div>
                </div>

                <!-- Финансы (выводы и резерв) -->
                <div id="admin-finance" class="admin-pane" style="display:none;">
                    <h3>💰 Управление финансами</h3>
                    <div style="margin-bottom:16px;">
                        <p><strong>Резерв проекта:</strong> <span id="reserveDisplay">${(reserve / 100).toFixed(2)} ⭐</span></p>
                        <button id="addReserveBtn" style="background:linear-gradient(135deg,#2b6e9e,#1a4c6e);">➕ Пополнить резерв</button>
                        <button id="subtractReserveBtn" style="background:rgba(255,0,0,0.2); border:1px solid #ff4444; color:#ff6b6b;">➖ Списать из резерва</button>
                    </div>
                    ...
                    <div id="withdrawalsList" style="max-height:400px; overflow-y:auto;">
                        ${withdrawals.map(w => `
                            <div style="background:rgba(0,0,0,0.3); border-radius:16px; padding:12px; margin-bottom:8px; display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:8px;">
                                <span>ID: ${w.user_id} | ${window.fromCents(w.amount)} ⭐ (комиссия ${window.fromCents(w.fee)} ⭐, к получению ${window.fromCents(w.receive)} ⭐) | ${w.status}</span>
                                ${w.status === 'pending' ? `
                                    <div>
                        ...
                        `).join('')}
                        ${!withdrawals.length ? '<p style="color:#9ca3af;">Нет заявок</p>' : ''}
                    </div>
                </div>
        ...
        // ---- ФИНАНСЫ (ВЫВОДЫ И РЕЗЕРВ) ----
        document.getElementById('addReserveBtn').onclick = async () => {
            const amount = prompt('Введите сумму пополнения резерва (в звёздах):');
            if (!amount) return;
            ...
            await window.addToReserve(window.toCents(stars));
            window.showToast('Резерв пополнен');
            window.renderAdminTab();
        };
        document.getElementById('subtractReserveBtn').onclick = async () => {
            const amount = prompt('Введите сумму списания из резерва (в звёздах):');
            if (!amount) return;
            const stars = parseFloat(amount);
            if (isNaN(stars) || stars <= 0) {
                window.showCustomModal('Ошибка', 'Введите корректную сумму');
            ...
            try {
                await window.subtractFromReserve(window.toCents(stars));
                window.showToast('Списано из резерва');
                window.renderAdminTab();
            } catch(e) {
                window.showCustomModal('Ошибка', e.message);
            }
        ...
        document.querySelectorAll('.approve-withdrawal').forEach(btn => {
            btn.addEventListener('click', async function() {
                ...
                try {
                    await window.adminApproveWithdrawal(id);
        ...
        document.querySelectorAll('.reject-withdrawal').forEach(btn => {
            btn.addEventListener('click', async function() {
                ...
                try {
                    await window.adminRejectWithdrawal(id);
                    window.showToast('Вывод отклонён');
                    window.renderAdminTab();
                } catch(e) {
                    window.showCustomModal('Ошибка', e.message);
                }
