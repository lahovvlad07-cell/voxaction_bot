window.renderAdmin = async () => {
    ...
    try {
        ...
        ordersDiv.innerHTML = '';
        if (ob.sell && ob.sell.length) ob.sell.forEach(o => ordersDiv.innerHTML += `<div class="order-item"><div>${window.fromCents(o.amount)} шт. по ${window.fromCents(o.price_per_share)} ⭐ (продавец ${o.seller_id})</div><button class="cancel-btn" data-id="${o.id}">Отменить</button></div>`);
        if (ob.buy && ob.buy.length) ob.buy.forEach(o => ordersDiv.innerHTML += `<div class="order-item"><div>${window.fromCents(o.amount)} шт. по ${window.fromCents(o.price_per_share)} ⭐ (покупатель ${o.buyer_id})</div><button class="cancel-btn" data-id="${o.id}">Отменить</button></div>`);
