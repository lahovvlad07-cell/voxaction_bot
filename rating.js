// rating.js – рабочий, с медалями и кликом
    ...
    document.querySelectorAll('.rating-item').forEach(item => {
        item.addEventListener('click', async () => {
            ...
            if (userId === currentUserId) {
                window.showCustomModal('Это вы', 'Перейдите на вкладку "Профиль" для редактирования.');
                return;
            }
            ...
            let totalVolume = trades ? trades.reduce((sum, t) => sum + (t.total_stars / 100), 0).toFixed(2) : '0.00';
                        ...
                        <div style="text-align:center; padding: 16px;">
                            ...
                            <div style="margin-top:16px;">
                                <div style="display:flex; gap:12px; justify-content:space-between;">
                                    <div class="stat-card" style="flex:1;"><div class="stat-value">${(user.stars_balance / 100).toFixed(2)}</div><div class="stat-label">Stars</div></div>
                                    <div class="stat-card" style="flex:1;"><div class="stat-value">${(user.shares / 100).toFixed(2)}</div><div class="stat-label">Акций</div></div>
                                </div>
                                <div style="display:flex; gap:12px; margin-top:12px;">
                                    <div class="stat-card" style="flex:1;"><div class="stat-value">${totalTrades}</div><div class="stat-label">Сделок</div></div>
                                    <div class="stat-card" style="flex:1;"><div class="stat-value">${totalVolume}</div><div class="stat-label">Объём (⭐)</div></div>
                                </div>
                            </div>
                        </div>
                    </div>
