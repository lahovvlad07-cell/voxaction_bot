// stocks.js – стабильная версия, без лишних наворотов (все функции рабочие)
...
function renderSellForm() {
    return `<div><input type="number" id="sellAmount" step="0.01" min="1" placeholder="Количество (от 1)"><input type="number" id="sellPrice" step="0.01" min="1" placeholder="Цена (от 1 Star)"><button id="sellBtn">➕ Выставить на продажу</button></div>`;
}
function renderBuyLimitForm() {
    return `<div><input type="number" id="buyAmountLimit" step="0.01" min="1" placeholder="Количество (от 1)"><input type="number" id="buyPriceLimit" step="0.01" min="1" placeholder="Цена (от 1 Star)"><button id="buyLimitBtn">💰 Выставить заявку на покупку</button></div>`;
}
...
window.renderStocksTab = async function(currentUser) {
    try {
        ...
        // процент изменения (опционально, для простоты пропустим)
        ...
        const html = `
            ...
            <div class="info-panel">
                <div class="info-card"><div class="label">🏦 Рыночная капитализация</div><div class="price">${Math.round(marketCap)} ⭐</div></div>
                <div class="info-card"><div class="label">📦 Всего акций</div><div class="price">${(totalShares/100).toFixed(2)}</div></div>
                <div class="info-card"><div class="label">📈 Средняя цена (24ч)</div><div class="price">${avg24h.toFixed(2)} ⭐</div></div>
            </div>
            ...
            <div><h3>📖 Стакан заявок</h3><div id="sellOrdersList"></div><div id="buyOrdersList"></div></div>
            <div><h3>📈 Продать акции</h3>${renderSellForm()}</div>
            <div><h3>🛒 Купить акции (лимитная заявка)</h3>${renderBuyLimitForm()}</div>
            <div><h3>📌 Мои ордера на продажу (${myOrders.length})</h3><div id="myOrdersList"></div>${myOrders.length ? '<button id="cancelAllSellsBtn">✖ Отменить все продажи</button>' : ''}</div>
            <div><h3>🛒 Мои заявки на покупку (${myBuyOrders.length})</h3><div id="myBuyOrdersList"></div>${myBuyOrders.length ? '<button id="cancelAllBuysBtn">✖ Отменить все покупки</button>' : ''}</div>
            <div><h3>📋 Ордера на продажу</h3><div id="ordersList"></div></div>
            <div><h3>🗂 История моих ордеров</h3><div id="orderHistoryList"></div></div>
        `;
        ...
        if (myOrders.length) {
            ...
            myDiv.innerHTML = myOrders.map(order => `<div class="order-card"><div>${window.fromCents(order.amount)} шт. по ${window.fromCents(order.price_per_share)} ⭐</div><button class="cancel-btn-small" data-id="${order.id}">Отменить</button></div>`).join('');
        ...
        if (myBuyOrders.length) {
            ...
            buyDiv.innerHTML = myBuyOrders.map(order => `<div class="order-card"><div>Купить ${window.fromCents(order.amount)} шт. по ${window.fromCents(order.price_per_share)} ⭐</div><button class="cancel-buy-btn" data-id="${order.id}">Отменить</button></div>`).join('');
        ...
        document.getElementById('cancelAllSellsBtn')?.addEventListener('click', async () => { if (confirm('Отменить все продажи?')) await cancelAllSellOrders(); });
        document.getElementById('cancelAllBuysBtn')?.addEventListener('click', async () => { if (confirm('Отменить все покупки?')) await cancelAllBuyOrders(); });
        ...
        document.getElementById('marketBuyBtn')?.addEventListener('click', () => {
            let stars = parseFloat(prompt('Сколько ⭐ потратить? (мин 1)', '100'));
            if (isNaN(stars) || stars < 1) return;
            marketBuy(stars).then(() => window.renderStocksTab(currentUser)).catch(e => window.showCustomModal('Ошибка', e.message));
        });
        document.getElementById('marketSellBtn')?.addEventListener('click', () => {
            let shares = parseFloat(prompt('Сколько акций продать? (мин 0.01)', '1'));
            if (isNaN(shares) || shares < 0.01) return;
            marketSell(shares).then(() => window.renderStocksTab(currentUser)).catch(e => window.showCustomModal('Ошибка', e.message));
        });
        subscribeToRealtime();
    } catch(e) {
        console.error(e);
